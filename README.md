# Não é só reta — Laboratório de Métricas Urbanas
...
Para deixar a execução mais reprodutível e mais rápida:

```bash
python scripts/build_graph.py \
  --place "Barão Geraldo, Campinas, Brazil" \
  --network drive \
  --out data/graph.graphml
```
...
├── src/nao_e_so_reta/
│   ├── analysis.py                # Erros, distorção, calibração de p
│   ├── config.py                  # Configurações do app
│   ├── graph_io.py                # Carregamento e projeção do grafo
│   ├── norms.py                   # Métricas Lp e geometria normativa
│   ├── projections.py             # Conversões CRS projetado <-> lat/lon
│   ├── routing.py                 # Nós próximos e menor caminho
│   ├── sampling.py                # Amostragem de pares de nós
│   ├── visualization.py           # Camadas Folium e legenda
│   └── ui_text.py                 # Textos explicativos da interface
├── scripts/
│   ├── build_graph.py             # Baixa e salva GraphML
│   └── calibrate_place.py         # Experimento de calibração
├── docs/
│   ├── matematica.md              # Fundamentos matemáticos
│   ├── arquitetura.md             # Arquitetura do sistema
│   ├── metodologia_experimentos.md
│   └── deploy.md
